...
const AppointmentCard = ({
  ...
  // Use dayjs to parse and handle dates
  const appointmentTime = dayjs(appointment.appointmentDateTime).utc(false);

  const currentTime = dayjs().utcOffset(5 * 60 + 45);
  // Check if the appointment is starting within the next 15 minutes
  ...
  // Format date for display in the local timezone, for user-friendly view
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm mb-4 flex justify-between items-center">
      ...
      <div className="flex space-x-4">
        {appointment.status === "upcoming" && !hasStarted && !isPast && (
          <Button type="primary" onClick={() => openModal("meetingLink")}>
            {appointment.meetingLink ? "Edit Meeting Link" : "Add Meeting Link"}
          </Button>
        )}
